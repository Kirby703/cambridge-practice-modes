...
PhantomPracticeGame.name = "Phantom Practice Chains"
PhantomPracticeGame.hash = "PhantomPracticeChains"
PhantomPracticeGame.tagline = "Keep still for 2 seconds to see the stack!"
...
function PhantomPracticeGame:whilePieceActive()
	if self.piece.lock_delay >= 120 then
		self.visible = true
	end
end
...
function PhantomPracticeGame:onPieceEnter()
	...
	if self.visible then
		...
		if self.chain > self.bestchain then
			self.bestchain = self.chain
		end
	end
	self.visible = false
end
...
function PhantomPracticeGame:drawScoringInfo()
	...
	love.graphics.printf("CHAIN", text_x, 200, 40, "left")
	love.graphics.printf("LEVEL", text_x, 320, 40, "left")
	love.graphics.printf("BEST", text_x, 260, 40, "left")
	...
	love.graphics.printf(self.chain, text_x, 220, 90, "left")
	love.graphics.printf(self.level, text_x, 340, 90, "left")
	love.graphics.printf(self.bestchain, text_x, 280, 90, "left")
	if self.clear then
